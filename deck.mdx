...
<CodeSurfer
  ...
  steps={[
    ...
    { lines: [1, 9], notes: "use `only` on describe to only run this block" },
    { lines: [6, 7, 8], notes: "use `only` on `test` or `it` to only run this test" },
...
<CodeSurfer
  title="Jest community packages"
  code={require("!raw-loader!./extended.js")}
  steps={[
    { notes: "Jest is already great, but nothing is perfect" },
